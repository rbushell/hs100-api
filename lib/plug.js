...
var commands = {
  ...
  setLightState: (state) => `{"smartlife.iot.smartbulb.lightingservice":{"transition_light_state":{"on_off":${(state ? 1 : 0)}}}}`,
  setColor: (hue,sat,bright) => `{"smartlife.iot.smartbulb.lightingservice":{"transition_light_state":{"saturation":${sat},"brightness":${bright},"mode":"normal","ignore_default":1,"color_temp":0,"transition_period":150,"on_off":1,"hue":${hue}}}}`
};
...
class Plug {
  ...
  setLightState (value) {
    return this.lb130_set(commands.setLightState(value));
  }

  setColor (hue,sat,brightness) {
    return this.lb130_set(commands.setColor(hue,sat,brightness));
  }

  lb130_set (cmd) {
    return this.set(cmd).then((data) => {
      try {
        var errCode = data['smartlife.iot.smartbulb.lightingservice'].transition_light_state.err_code;
        return (errCode === 0);
      } catch (e) {}
      if (errCode !== 0) { throw data; }
    });
